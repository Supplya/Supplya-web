...
export class EditBlogPostComponent implements OnInit {
  ...
  getPostByID() {
    ...
    this.loading = true;
    this.productService.getPostByID(this.id).subscribe(
      (details: any) => {
        if (details.status) {
          this.response = details?.data;
          this.loading = false;
this.setInitialTags(this.response?.tags);
          // Extract category _id
          const formData = {
            ...details.data,
            // category: details.data.category._id,
          };
  ...
  submit() {
    ...
    if (this.form.valid && this.images.length > 0) {
      this.productService.updateBlogPost(this.response?._id, formData).subscribe((data) => {
        if (data.status) {
